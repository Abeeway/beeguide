...
The number of outdated entries — where the most recent almanac entry for a given satellite is older than 60 days — is counted per GNSS chip and per constellation. The result is provided as a bitmap as part of periodic [status messages](./application-uplink.md#response-of-a-system-status-request) in order to allow a back-end application to proactively update the outdated entries using [almanac update downlink commands](./application-downlink.md#update-gps-almanac). it is also shown in the output of CLI command `gnss info`.
