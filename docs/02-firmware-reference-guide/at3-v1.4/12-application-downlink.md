...
The data part is a list of parameters as defined below. Note that the format is the same as the [](./10-application-uplink.md#parameter-class-configuration-get-response).
...
| Binary | File path on the remote server |
|--------|--------------------------------|
| 1 byte | Variable                       |
...
- **Binary**:
  - 0 – Application.
  - 1 – BLE stack.
  - 2 – MT3333 (for future use).
  - 3 – LR11XX (for future use).
- **File path**: Path to the signed binary on the remote server, must be
- provided as a null-terminated string.

The tracker responds with a
[`FUOTA response`](./10-application-uplink.md#fuota-response).

:::note
Because the request can be transmitted over the LoRa network, it must
comply with the LoRaWAN downlink payload size limit of 51 bytes.
Consequently, the maximum supported file path length is limited to 47
bytes (characters).
:::

## Responses

This section outlines the payload format of responses to the [device queries](./10-application-uplink.md#device-queries).
...
This downlink replies to an [echo-request](./10-application-uplink.md#echo-request) query from the tracker (only used over a cellular network).
...
This downlink frame is used to setup GPS almanac entries. It can be used as an asynchronous command (a server can monitor the status of GNSS almanacs through status notifications), or as response to a [request](./10-application-uplink.md#update-gps-almanac) query initiated by the tracker.
