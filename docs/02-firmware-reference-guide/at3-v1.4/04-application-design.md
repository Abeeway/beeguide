...
![Application design](./images/media/app-design-1.png)
...
- **Core**  
  ...
  - An [**event manager**](./05-event-manager.md) that distributes application events to registered clients.
  - A [**geolocation manager**](./06-geoloc-manager.md) that schedules position acquisitions.
  - A [**GNSS manager**](./07-gnss-manager.md) that manages almanacs and aiding positions.
  - A [**network manager**](./08-networking.md) that handles networking using LoRaWAN and LTE.
  - A [**payload manager**](./09-payload-manager.md) that generates unsolicited uplinks (position, notifications).
  - A [**device manager**](./13-device-power-monitoring.md) that monitors system health (temperature, battery level, etc.).
  - A **Command Line Interface (CLI)** that allows system configuration and monitoring.
  - A [FUOTA manager](./15-firmware-update.md) handling firmware updates over the air
