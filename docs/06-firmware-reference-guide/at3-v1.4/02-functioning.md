...
This section describes the high-level features of AT3 V1.4 release.
...
This "Join-then-activate mode" is useful for consumer products to avoid any manual action to activate the device, which is entirely controlled by the presence of a valid LoRaWAN network subscription for the device.
...
When the device is in the **Active** state, it can transition to the **Off** state only if at least one of the two button event maps contains the `device_stop` action or —if the device is rechargeable— when the battery is in critical low condition.  

Before entering the **Off** state, the device passes through the transitional state **Shutting-down**, during which it sends shut-down uplink messages over the network.

In the **Off** state, the following features are disabled:

- BLE beaconing
- BLE connectivity
- Accelerometer
- Geolocation (ongoing acquisitions are cancelled)
- Network (LoRaWAN and cellular)
...
![Geozoning area types](./images/media/image2.png)
...
The beacon identifier can be configured using the firmware parameters. For more details on BLE beaconing, please refer to [BLE group](./16-configuration.md#ble-group).
...
- **accelero_motion_duration**: This application firmware generates
    [motion end notification](./application-uplink#notification-class-3-accelerometer) once this timer
