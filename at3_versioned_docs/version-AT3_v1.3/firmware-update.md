...
As of AT3 v1.3, the application firmware can also be updated:
- via BLE.
...
You must use the 'Beehive' tool available at https://abeeway.io or https://nano-things.net/beehive/#
...
:::warning
This is a preview feature in AT3 v1.3, available only from command line.
The release version will operate automatically from a FUOTA server and add a downlink command to trigger a over-the-air upgrade.
:::
...
- **Phase 1. Binary file download**  
  The firmware binary is downloaded over the cellular network using **HTTPS** (TLS 1.2 with Basic Authentication), ensuring encrypted and authenticated communication.  

  The device stores the firmware segments to external flash memory. The download is **resilient to temporary disconnections** and will resume from the last completed offset.  

  Once the download is complete:
  - AT3 verifies that the binary type matches the platform type (e.g., Compact Tracker, Combo Cellular Compact Tracker, Abeeway 1WL EVK).  
