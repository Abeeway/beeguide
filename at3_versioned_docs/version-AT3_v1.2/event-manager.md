...
:::note
The event `type_network_cellular_lpm_changed` is triggered when the power consumption of the LTE module changes depending on the low power mode (LPM) accepted by the network. For example, during an initial connection attempt, the LTE module requests to use PSM mode as its LPM. If the network accepts this mode, a `type_network_cellular_lpm_changed` event is generated. During testing, the tracker is located in a geographical area where a cell does not support PSM. The application processor then receives a "URC" (Unsolicited Result Code) notification from the cellular modem indicating that PSM is no longer supported. This again triggers a `type_network_cellular_lpm_changed` event, which accurately reflects the change in LPM mode of the LTE module.
:::
...
| type_core_dl_trigger      | 20  | Instructs the system to send a Heartbeat message to open a downlink Rx slot. |
