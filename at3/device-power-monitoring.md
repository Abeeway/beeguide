...
***type_power_battery_low*** to the system to signal a critically low
level.
...
When changing the primary battery, the user should update the estimation
stored in the internal group (either clear it or set it to a new value)
via a downlink or by using a dedicated CLI command. Based on this new
estimation value, the system will check whether there is enough energy
in the battery:
- Send the event ***type_power_battery_low*** if the remaining energy is
below 5%.
- Send the event ***type_power_battery_normal*** if the remaining energy
is above or equal to 5%.

In the case of a rechargeable battery, setting the estimation value in
the internal group has no effect.
